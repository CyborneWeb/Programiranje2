...
public class Vaje3 {
    ...
    static void izpisi64bit(long[] nizZnakov){
        for(int vrstica=0; vrstica < 8; vrstica++) {
            for (int znak = 0; znak < nizZnakov.length; znak++) {
                if (znak == 0) System.out.print(belaPika);
    ...
    static void izpisi64bit(String niz){
        long[] kode = new long[niz.length() + 2];
        niz = niz.toUpperCase();
        ...
        } kode[niz.length()] = 0; kode[niz.length() + 1] = 4342219536296657468L;
        izpisi64bit(kode);
    }
...
//        izpisi64bit(-36525672788885761L);
        if (args.length > 0){
            ...
            if (velikost == 4){
                ...
                izpisi64bit(niz);
            } else System.out.println("Neveljavna velikost: prvi argument naj bo 4 ali 8");
        }
    }
}
